...
const envSchema = z.object({
  ...
  OPENROUTER_VISION_MODEL: z.string().min(1).default('google/gemini-3.1-flash-lite'),
  OPENROUTER_TIMEOUT_MS: z.coerce.number().int().positive().default(20_000),
  PUBLIC_APP_URL: z.string().url().default('http://localhost:5180'),
  MEDIA_BUCKET_ENDPOINT: z.preprocess((value) => value === '' ? undefined : value, z.string().url().optional()),
  MEDIA_BUCKET_NAME: z.preprocess((value) => value === '' ? undefined : value, z.string().min(1).optional()),
  MEDIA_BUCKET_ACCESS_KEY_ID: z.preprocess((value) => value === '' ? undefined : value, z.string().min(1).optional()),
  MEDIA_BUCKET_SECRET_ACCESS_KEY: z.preprocess((value) => value === '' ? undefined : value, z.string().min(1).optional()),
